...
import { BookOpen, Zap, Award, Briefcase, Mail, Lock, User, Eye, EyeOff } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen flex">
      ...
      >
        ...
        <p className="text-white/40 text-xs relative z-10">© 2026 SkillForge. All rights reserved.</p>
      </motion.div>
      ...
      <div className="flex-1 flex items-center justify-center p-6 bg-background">
        ...
        >
          ...
          <p className="text-muted-foreground text-sm mb-8">
            {mode === 'signin' ? 'Sign in to continue to SkillForge' : 'Start your learning journey today'}
          </p>

          {mode === 'signin' ? (
            <SignInForm showPassword={showPassword} setShowPassword={setShowPassword} onSwitch={() => setMode('register')} />
          ) : (
            <RegisterForm showPassword={showPassword} setShowPassword={setShowPassword} showConfirm={showConfirm} setShowConfirm={setShowConfirm} onSwitch={() => setMode('signin')} />
          )}
        </motion.div>
      </div>
    </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-center text-sm text-muted-foreground">
        ...
        <button onClick={onSwitch} className="text-primary font-medium hover:underline">Sign Up</button>
      </p>
    </div>
  );
}

function RegisterForm({ showPassword, setShowPassword, showConfirm, setShowConfirm, onSwitch }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <div className="relative">
          <Lock className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
          <Input type={showConfirm ? 'text' : 'password'} placeholder="••••••••" className="pl-9 pr-9" />
