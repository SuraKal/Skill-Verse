...
export function InvitationLandingPage({ token }: { token: string | null }) {
  ...
  const { invitationType, token: invitationToken } = useParams<{
    invitationType: "organization" | "course" | "course-enrollment";
    token: string;
  }>();
  ...
  const normalizedType =
    invitationType === "course"
      ? "course_instructor"
      : invitationType === "course-enrollment"
        ? "course_enrollment"
        : "organization";
  ...
  const eyebrowLabel =
    normalizedType === "organization"
      ...
      : normalizedType === "course_instructor"
        ? "Course instructor invitation"
        : "Course enrollment invitation";

  const descriptionText = invitation
    ? normalizedType === "organization"
      ...
      : normalizedType === "course_instructor"
        ...
        : `You were invited to enroll in ${invitation.course_title} for ${invitation.organization_name}.`
        ...
        <main className="sv-public-main">
          ...
          >
            <div
              ...
              style={{ height: 24, width: 220, marginBottom: 12 }}
            />
            <div
              className="sv-skeleton"
  ...
  return (
    <div className="sv-public-shell">
      ...
      <main className="sv-public-main">
        <div
          className="sv-panel"
          style={{ maxWidth: 760, margin: "48px auto" }}
        >
          <div className="sv-page-header">
            <p className="sv-section__eyebrow">{eyebrowLabel}</p>
            <h1 className="sv-page-header__title">
              {invitation?.title ?? "Invitation unavailable"}
            </h1>
            <p className="sv-page-header__sub">{descriptionText}</p>
          </div>

          {intendedAction && invitation ? (
            <p
              className="sv-inline-note"
              style={{ marginTop: 0, marginBottom: 12 }}
            >
              Use the buttons below to{" "}
              {intendedAction === "accept" ? "accept" : "decline"} this
              invitation.
            </p>
          ) : null}
            ...
            <>
              <div className="sv-grid-2" style={{ marginBottom: 12 }}>
                <div className="sv-selector-card" style={{ marginTop: 0 }}>
                  <div className="sv-selector-card__title">
                    Invitation details
                  </div>
                ...
                {(normalizedType === "course_instructor" ||
                  normalizedType === "course_enrollment") &&
                invitation.custom_message ? (
                  <div className="sv-selector-card" style={{ marginTop: 0 }}>
                    ...
                    <p
                      className="sv-course-card__description"
